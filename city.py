...
class CityScape(object):
	...
	def add_acs(self, acs):
		...
		for i,r in self.gdf.iterrows():
			...
			candidate_tract = [int(id) for id in idx.intersection(geom.bounds)]
		...
		#add except for geography
		acs = acs.drop(columns=['geometry'])
		self.gdf = self.gdf.merge(acs, left_on='tract', right_on='GEOID')
	...
	def chicago(cls, n=-1):
		...
		gdf = gpd.read_file('gis_data/acs')
		city.add_acs(gdf)
		return city
