...
def get_chicago_parks_distance():
    ...
    parks = pd.DataFrame.from_dict(data, orient='columns')
    ...
    parks['latitude'] = parks.apply(lambda x: parse(x, 'latitude'), axis=1)
    parks['longitude'] = parks.apply(lambda x: parse(x, 'longitude'), axis=1)

    parks = parks.dropna(subset=['latitude', 'longitude'])

    gdf = gpd.GeoDataFrame(parks, geometry=gpd.points_from_xy(parks.latitude, parks.longitude))
